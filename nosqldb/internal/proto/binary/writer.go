...
// WriteLimitsMode encodes and writes the limits mode value to the buffer.
func (w *Writer) WriteLimitsMode(lm types.LimitsMode, serialVersion int16) (int, error) {
	if serialVersion <= 2 {
		return 0, nil
	}
	return w.writeOneByte(w.getLimitsMode(lm))
}
...
// getLimitsMode returns the limits mode value accepted by the server, which is
// 1 for provisioned and 2 for autoscaling.
func (w *Writer) getLimitsMode(lm types.LimitsMode) byte {
	if lm == types.AutoScaling {
		return 2
	}
