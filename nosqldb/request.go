...
// and mode (provisioned or autoscaling) desired for the table.
//
...
type TableRequest struct {
	...
	// mode (provisioned or autoscaling) for the table.
	//
...
type TableLimits struct {
	...
	// LimitsMode specifies if the table is provisioned (the default) or
	// autoscaling.
	LimitsMode types.LimitsMode `json:"limitsMode"`
}
...
func ProvisionedTableLimits(RUs uint, WUs uint, GB uint) *TableLimits {
	return &TableLimits{
		...
		WriteUnits: WUs,
		StorageGB:  GB,
		LimitsMode: types.Provisioned,
	}
}

// AutoScalingTableLimits returns a TableLimits struct set up for
// autoscaling (flexible read/write limits) tables.
func AutoScalingTableLimits(GB uint) *TableLimits {
	return &TableLimits{
		ReadUnits:  0,
		WriteUnits: 0,
		StorageGB:  GB,
		LimitsMode: types.AutoScaling,
	}
}

func (l *TableLimits) validate() (err error) {
	if l.LimitsMode != types.Provisioned && l.LimitsMode != types.AutoScaling {
		return nosqlerr.NewIllegalArgument("TableLimits LimitsMode must be one of " +
			"Provisioned or AutoScaling")
	}
	if l.StorageGB == 0 {
		return nosqlerr.NewIllegalArgument("TableLimits StorageGB must be positive")
	}
	if l.LimitsMode == types.Provisioned {
		if l.ReadUnits == 0 || l.WriteUnits == 0 {
			return nosqlerr.NewIllegalArgument("TableLimits read/write units must be positive")
		}
	} else {
		if l.ReadUnits != 0 || l.WriteUnits != 0 {
			return nosqlerr.NewIllegalArgument("TableLimits read/write units must be zero for AutoScaling table")
		}
	}
	return nil
}
