...
func (req *TableRequest) serialize(w proto.Writer, serialVersion int16) (err error) {
	...
	if _, err = w.WriteLimitsMode(req.TableLimits.LimitsMode, serialVersion); err != nil {
		return
	}
...
func readLimitsMode(r proto.Reader, serialVersion int16) (types.LimitsMode, error) {
	if serialVersion <= 2 {
...
func deserializeTableResult(r proto.Reader, serialVersion int16) (*TableResult, error) {
	...
	if hasStaticState {
		...
		mode, err := readLimitsMode(r, serialVersion)
		if err != nil {
			return res, err
		}
